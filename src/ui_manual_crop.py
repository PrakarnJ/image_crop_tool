"""Manual crop tab: toolbar, status bar, navigation, folder pickers, and
...
class ManualCropTab(ttk.Frame):
    def __init__(self, notebook: ttk.Notebook, root: tk.Tk):
        ...
        notebook.add(self, text="Manual Crop")
        self._root = root
    ...
    def _build_ui(self) -> None:
        ...
        self.canvas = CanvasView(self, on_change=self._update_status)
    ...
    def _setup_shortcuts(self) -> None:
        self._root.bind("<n>", lambda _e: self.next_image())
        self._root.bind("<p>", lambda _e: self.previous_image())
        self._root.bind("<s>", lambda _e: self.save_crops())
        self._root.bind("<u>", lambda _e: self.undo())
        self._root.bind("<c>", lambda _e: self.clear_all())
        self._root.bind("<Escape>", lambda _e: self.canvas.cancel_drag())
    ...
    def save_crops(self, show_message: bool = True) -> int:
        ...
        if show_message:
            ...
            messagebox.showinfo("Saved", f"Saved {count} crop{plural} to {self.output_folder}")
        return count
    ...
    def previous_image(self) -> None:
        if not self.image_paths:
            return
        if self.canvas.num_rects() > 0 and not self.saved_for_current:
            if not messagebox.askyesno("Unsaved crops", "You have unsaved crops on this image. Go back anyway?"):
                return
